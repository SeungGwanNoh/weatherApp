$(document).ready(function(){
    var city = []; //사용자가 클릭한 곳의 도시명을 숫서대로 넣을 공간 (사용자가 좌측부터 클릭한다는 보장을 할 수 없기 때문)
    var myKey = "15f5a90d387ec4c8893ae258c06ba69a";
    var state_icon = "";
    ...
    `;
    //배열 데이터의 개수를 활용하요 각 도시별 날씨 정보를 요청할 때마다 받는다면
    function w_info(){
        //배열 데이터의 개수와는 상관없이 날씨의 개별 박스를 모두 제거
        $("#weather ul").empty();

        //현재 시점에서 배열의 개수만큼 다시 반복하여 생성해라(renewel - 갱신)
    ...
    function search(){
        ...
        if($search_val.length < 1){ //입력값이 존재하지 않을 때와 동일
            ...
            //소문자 작성인지 또는 대문자 작성인지 모름 => 모든 글자를 소문사로 변경
...
//api.openweathermap.org/data/2.5/weather?q=London&appid=15f5a90d387ec4c8893ae258c06ba69a
//15f5a90d387ec4c8893ae258c06ba69a